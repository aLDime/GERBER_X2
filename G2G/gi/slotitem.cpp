...
SlotItem::SlotItem(const Path& path, double size, DrillFile* file)
    : m_file(file)
    , m_size(size)
{
    m_paths = { path };
    ...
    offset.Execute(tmpPpath, size * 0.5 * uScale);
...
int SlotItem::type() const { return SlotItemType; }

Paths SlotItem::paths() const { return m_paths; }

QPainterPath SlotItem::shape() const { return m_shape; }

const DrillFile* SlotItem::file() const { return m_file; }

//SlotItem::SlotItem(GraphicObject& item)
...
//void SlotItem::paint(QPainter* painter,
