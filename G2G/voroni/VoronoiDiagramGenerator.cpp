...
bool VoronoiDiagramGenerator::ELinitialize()
{
    ...
    for (i = 0; i < ELhashsize; i += 1)
        ELhash[i] = (struct Halfedge*)nullptr;
...
void VoronoiDiagramGenerator::PQinsert(struct Halfedge* he, struct Site* v, float offset)
{
    ...
    while ((next = last->PQnext) != (struct Halfedge*)nullptr && (he->ystar > next->ystar || (he->ystar == next->ystar && v->coord.x > next->vertex->coord.x))) {
        last = next;
    };
...
struct Point VoronoiDiagramGenerator::PQ_min()
{
    ...
    while (PQhash[PQmin].PQnext == (struct Halfedge*)nullptr) {
        PQmin += 1;
    };
...
bool VoronoiDiagramGenerator::PQinitialize()
{
    ...
    for (i = 0; i < PQhashsize; i += 1)
        PQhash[i].PQnext = (struct Halfedge*)nullptr;
...
void VoronoiDiagramGenerator::line(float x1, float y1, float x2, float y2) { pushGraphEdge(x1, y1, x2, y2); }
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
    ...
    while (1) {
        ...
        if (newsite != (struct Site*)nullptr && (PQempty() || newsite->coord.y < newintstar.y || (newsite->coord.y == newintstar.y && newsite->coord.x < newintstar.x))) { /* new site is smallest - this is a site event*/
